...
@interface SHFSingleDownloader()<NSURLSessionDelegate,NSURLSessionDownloadDelegate>
//下载任务
...
@implementation SHFSingleDownloader
- (instancetype)init
{
    self = [super init];
    if (self) {
        ...
        self.downloaderState = SHFDownloaderStateUnStart;
...
- (void)pauseisHand:(BOOL)isHand {
    ...
    if (self.downloadTask.state == NSURLSessionTaskStateRunning) {
        ...
        if (self.downloaderState != SHFDownloaderStateDeleted) {
            self.downloaderState = SHFDownloaderStatePause;
            [self.downloaderDelegate downloaderState:self.downloaderState andDownloaderUrl:self.downloadUrl];
        }
...
- (void)resumeisHand:(BOOL)isHand {
    if ([[self judgeNetworkType] isEqualToString:networkUnknown] || [[self judgeNetworkType] isEqualToString:networkINotReachable]) {
        ...
        if (self.isConcede == YES) {
            self.isHand = isHand;
            self.downloaderState = SHFDownloaderStateRunning;
            [self.downloaderDelegate downloaderState:self.downloaderState andDownloaderUrl:self.downloadUrl];
        ...
        if (self.downloadTask.state == NSURLSessionTaskStateRunning || self.downloadTask.state == NSURLSessionTaskStateCompleted || self.downloadTask.state == NSURLSessionTaskStateCanceling) {
            ...
            if (self.downloaderState == SHFDownloaderStateFail) {
...
- (NSString *)judgeNetworkType {
    return [[NSUserDefaults standardUserDefaults] objectForKey:SHFCurrentNetworkType];
}
...
- (void)judgeDownloaderStateToHandel {
    ...
    self.downloaderState = SHFDownloaderStateDeleted;
    [self.downloaderDelegate downloaderState:self.downloaderState andDownloaderUrl:self.downloadUrl];
}

#pragma mark - 取消当前下载的线程，让路其他线程
- (void)cancelRorOtherDownloader {
    if (self.downloaderState != SHFDownloaderStateRunning) {
        NSLog(@"此状态不允许暂停");
        return;
    }
    self.isConcede = YES;
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didFinishDownloadingToURL:(NSURL *)location {
    NSLog(@"文件下载完成");
    NSString *path = [NSString stringWithFormat:@"%@.%@", [[[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) objectAtIndex:0] stringByAppendingPathComponent:@"SHFDownloads"]stringByAppendingPathComponent:[SHFTool encodeFilename:self.downloadUrl]], self.fileType];
    NSLog(@"文件路径是:%@", path);
    ...
    //告知下载控制器已经系在完成
    self.downloaderState = SHFDownloaderStateSuccess;
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didWriteData:(int64_t)bytesWritten totalBytesWritten:(int64_t)totalBytesWritten totalBytesExpectedToWrite:(int64_t)totalBytesExpectedToWrite {
    ...
    if (self.downloaderState != SHFDownloaderStateDeleted) {
        ...
        if (self.downloaderState != SHFDownloaderStateRunning) {
            if (self.isSendState == YES) {
                [self.downloaderDelegate downloaderState:SHFDownloaderStateRunning andDownloaderUrl:self.downloadUrl];
            }
        }
    ...
    if (error) {
        ...
        if (self.downloadTask == nil || [self.downloadTask isEqual:[NSNull null]]) {
            ...
            if ([error.userInfo objectForKey:NSURLSessionDownloadTaskResumeData]) {
                ...
                //没有继续下载的数据
                if (self.downloaderState != SHFDownloaderStateDeleted) {
                    self.downloaderState = SHFDownloaderStateFail;
                    self.isHand = YES;
                    [self.downloaderDelegate downloaderState:self.downloaderState andDownloaderUrl:self.downloadUrl];
                }
                NSLog(@"没有继续下载的数据");
...
- (void)updateLocalResumeData {
    ...
    //同时保存在本地一份
    //获取存储路径
    NSString *path = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) objectAtIndex:0] stringByAppendingPathComponent:@"SHFResumeDownloads"];
    //获取文件名
    NSString *resumeFileName = [path stringByAppendingPathComponent:[@"resume_" stringByAppendingString:[SHFTool encodeFilename:self.downloadUrl]]];
    //存储数据
...
- (NSString *)unDownloadStr {
    if (_unDownloadStr == nil) {
        _unDownloadStr = [[[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) objectAtIndex:0] stringByAppendingPathComponent:@"SHFUnDownloads"] stringByAppendingPathComponent:self.tmpFilename];
    }
...
- (NSString *)resumeDirectoryStr {
    if (_resumeDirectoryStr == nil) {
        _resumeDirectoryStr = [[[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) objectAtIndex:0] stringByAppendingPathComponent:@"SHFResumeDownloads"] stringByAppendingPathComponent:[@"resume_" stringByAppendingString:[SHFTool encodeFilename:self.downloadUrl]]];
    }
