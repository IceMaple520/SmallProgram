...
static SHFDownloader *_downloader;
static dispatch_once_t onceToken1;
static dispatch_once_t onceToken2;

@interface SHFDownloader()<SHFSingleDownloaderDelegate>
//存储下载数据的路径
...
//用于存储原始的从数据库中读取的下载信息
@property (strong, nonatomic) RLMResults<SHFSingleDownloaderModel *> *allModels;
...
+ (instancetype)sharedDownloader {
    dispatch_once(&onceToken1, ^{
        if (_downloader == nil) {
            _downloader = [[SHFDownloader alloc] init];
        }
    });
    return _downloader;
}
...
- (instancetype)init {
    ...
    if (self) {
        ...
        //开启网络监测
        [SHFJudgeNetworkType judgeNetworkTypeIs:^(NSString *networkType) {
...
+ (void)destoryDownloader {
    ...
    for (SHFSingleDownloader *downloader in _downloader.singleDownloaderArray) {
...
- (void)startDownloadWithDownloadUrl:(NSString *)downloadUrl filename:(NSString *)filename fileType:(NSString *)fileType isHand:(BOOL)isHand {
    ...
    if ([SHFTool checkIsUrlAtString:downloadUrl]) {
        //有效
        NSLog(@"有效的网址");
    } else {
        //无效
    ...
    //判断文件名和文件类型是否为空
    if ([SHFTool checkIsEmpty:filename] || [SHFTool checkIsEmpty:fileType]) {
        //空
    ...
    //检查下载器是否已经存在
    __block SHFSingleDownloader *seekDownloader = nil;
    __block BOOL isD = NO;
    [self.singleDownloaderArray enumerateObjectsUsingBlock:^(SHFSingleDownloader *_Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    if (isD) {
        ...
        if (seekDownloader.downloaderProgress >= 1.0) {
            ...
            if ([self.zylDownloaderDelegate respondsToSelector:@selector(downloaderState:andDownloaderUrl:)]) {
                [self.zylDownloaderDelegate downloaderState:SHFDownloaderStateSuccess andDownloaderUrl:seekDownloader.downloadUrl];
            }
        ...
        SHFSingleDownloader *singleDownloader = [[SHFSingleDownloader alloc] init];
...
/*************************************************************************/
- (void)addDownloader:(SHFSingleDownloader *)downloader isHand:(BOOL)isHand isControl:(BOOL)isControl{
    //首先判断是不是手动开启新的下载
    if (isHand) {
        //是手动,强行开启下载
        //判断是否达到最大下载数目
        if (self.downloadingArray.count < self.maxDownloaderNum) {
            ...
            //达到了
            //暂停最前面的正在下载
            SHFSingleDownloader *firstDownloader = [self.downloadingArray firstObject];
...
- (void)removeDownloader:(SHFSingleDownloader *)downloader isHand:(BOOL)isHand isControl:(BOOL)isControl {
    __block BOOL isE = NO;
    [self.downloadingArray enumerateObjectsUsingBlock:^(SHFSingleDownloader *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    if (isE) {
        ...
        if (downloader.downloaderProgress >= 1.0) {
            ...
            if (![self.waitingDownlodArray containsObject:downloader]) {
                ...
                if (downloader.downloaderState != SHFDownloaderStateDeleted) {
                    [self.waitingDownlodArray addObject:downloader];
                } else if (downloader.downloaderState == SHFDownloaderStateDeleted) {
                    if ([self.waitingDownlodArray containsObject:downloader]) {
                        [self.waitingDownlodArray removeObject:downloader];
                    }
                }
...
- (void)checkDownloadProgressExceptDownloader:(SHFSingleDownloader *)downloader {
    //判断正在下载的数组中是否有空缺
    if (self.downloadingArray.count < self.maxDownloaderNum) {
        //有空缺
        //检查等待下载的数组中是否有数据
        if (self.waitingDownlodArray.count > 0) {
            //有
            //寻找第一个需要下载的数据
            __block SHFSingleDownloader *firstDownloader = nil;
            if (downloader == nil) {
                [self.waitingDownlodArray enumerateObjectsUsingBlock:^(SHFSingleDownloader *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    if (obj.isHand == NO) {
                        firstDownloader = obj;
                        *stop = YES;
                    }
...
- (void)saveDownloaderInfoWithSingleDownloader:(SHFSingleDownloader *)singleDownloader {
    //创建存储对象
    SHFSingleDownloaderModel *model = [[SHFSingleDownloaderModel alloc] init];
...
- (NSArray *)readDownloadersFromRealm {
    self.allModels = [SHFSingleDownloaderModel allObjects];
    _singleDownloaderArray = [[NSMutableArray alloc] init];
    if (self.allModels.count > 0) {
        //数据库有数据
        for (SHFSingleDownloaderModel *model in self.allModels) {
            SHFSingleDownloader *singDownloader = [[SHFSingleDownloader alloc] init];
...
- (SHFSingleDownloaderModel *)getDownloaderInfoWithDownloaderUrl:(NSString *)downloaderUrl {
    //首先判断下载连接是否在数据数组中
    SHFSingleDownloaderModel *targetModel = [[SHFSingleDownloaderModel alloc] init];
    __block BOOL isD = NO;
    __block SHFSingleDownloader *downloaderModel = nil;
    [self.singleDownloaderArray enumerateObjectsUsingBlock:^(SHFSingleDownloader *_Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    if (isD) {
        ...
        if (downloaderModel.isExistInRealm == YES) {
            ...
            if (downloaderModel.downloaderProgress >= 1.0) {
                ...
                NSString *localUrl = [self.directoryStr stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.%@", [SHFTool encodeFilename:downloaderModel.downloadUrl], downloaderModel.fileType]];
...
- (NSArray<SHFSingleDownloaderModel *> *)getAllDownloadersInfo {
    ...
    [self.singleDownloaderArray enumerateObjectsUsingBlock:^(SHFSingleDownloader*  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        SHFSingleDownloaderModel *model = [self getDownloaderInfoWithDownloaderUrl:obj.downloadUrl];
        if (model != nil) {
...
- (void)deleteDownloaderInfoWithDownloderUrl:(NSString *)downloaderUrl {
    ...
    [self.singleDownloaderArray enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(SHFSingleDownloader *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([downloaderUrl isEqualToString:obj.downloadUrl]) {
            //已经存在这个下载了
            isD = YES;
            downloaderModel = obj;
    ...
    if (isD) {
        ...
        if (downloaderModel.isExistInRealm == YES) {
            ...
            //②继续下载的数据
            NSString *resumeDataStr = [self.resumeDirectoryStr stringByAppendingPathComponent:[@"resume_" stringByAppendingString:[SHFTool encodeFilename:downloaderModel.downloadUrl]]];
...
- (void)deleteDownloaderFromReaml:(SHFSingleDownloader *)downloader {
    //创建存储对象
    for (SHFSingleDownloaderModel *model in self.allModels) {
    ...
    if (isE) {
        ...
        if (downloaderModel.isExistInRealm) {
            ...
            if (downloaderModel.downloaderProgress >= 1.0) {
                ...
                if ([self.fileManager fileExistsAtPath:localUrl]){
                    //根据新的文件信息更新文件名
                    NSString *newLocalUrl = [self.directoryStr stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.%@", [SHFTool encodeFilename:downloaderModel.downloadUrl], downloaderModel.fileType]];
                    NSError *error = nil;
...
- (void)downloaderState:(SHFDownloaderState)state andDownloaderUrl:(NSString *)downloaderUrl {
    ...
    //找到对应的下载器
    __block SHFSingleDownloader *downloader = nil;
    [self.singleDownloaderArray enumerateObjectsUsingBlock:^(SHFSingleDownloader *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj.downloadUrl isEqualToString:downloaderUrl]) {
            downloader = obj;
            *stop = YES;
